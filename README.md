...
- **Noise Reduction:** Salt & Pepper 노이즈 제거를 위한 Mean Filter와 Median Filter 비교 분석 (Edge-padding 적용)
...
이미지의 고유한 특징점을 추출하고 이를 매칭하여 여러 장의 사진을 하나의 광각 파노라마로 합성하는 엔드투엔드(End-to-End) 파이프라인을 구축했습니다.
...
- **SIFT-like Descriptor:** 특징점 주변의 기울기 크기(Magnitude)와 방향(Orientation) 히스토그램을 생성하여 조명이나 미세한 변화에도 강인한 128차원 기술자를 추출하는 로직을 설계했습니다.
- **Robust Matching & RANSAC:** NNDR(Nearest Neighbor Distance Ratio) 매칭을 통해 1, 2순위 후보 간의 모호성을 제거하고, RANSAC 알고리즘으로 잘못 매칭된 점(Outliers)을 걸러내어 최적의 투영 변환 행렬(Homography)을 추정했습니다.
...
* **Algorithm Comparison (SIFT vs SURF):** 가우시안 블러 기반의 SIFT와 적분 영상(Integral Image)을 활용한 SURF의 원리를 비교하며, 연산 속도와 회전/스케일 불변성 사이의 기술적 차이점을 학습했습니다.

---

### 3️⃣ Bag-of-Visual-Words Image Classification
전통적인 컴퓨터 비전 기법인 Bag-of-Visual-Words(BoVW) 파이프라인을 구축하여 비행기(Airplanes)와 자동차(Cars) 객체를 분류하는 이진 분류기를 구현했습니다.
...
- **Advanced Optimization:** 단순 SGD를 넘어 **AdamW 옵티마이저**와 가중치 감쇠(Weight Decay)를 적용하여 수렴 속도와 일반화 성능을 동시에 확보했습니다.
- **Deepening & Blocks:** 2-레이어 베이스라인 모델에서 시작하여, 연산 효율을 위한 **VGG 스타일의 6-레이어 블록 구조**로 고도화하여 모델의 표현력을 확장했습니다.
- **Regularization Techniques:** 학습 안정성을 위한 **Batch Normalization**과 과적합(Overfitting) 방지를 위한 **Dropout(0.5)**을 도입하여 모델의 신뢰도를 높였습니다.
...
* **클래스별 불균형 해소:** 초기 모델에서 혼동이 잦았던 '고양이(Cat)'와 '강아지(Dog)' 클래스의 정확도를 높이기 위해 모델의 깊이를 늘리고 BatchNorm을 추가하여, 미세한 특징 차이를 잡아낼 수 있도록 개선했습니다.
